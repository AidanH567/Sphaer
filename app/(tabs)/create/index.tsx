...
export default function CreateScreen() {
  ...
  // Set when the cover image came from the poster generator rather than the
  // photo library. Holds the PNG bytes so publish can upload them directly —
  // `posterUri` is only the data: URI used to preview the same image.
  ...
  // The poster generator reads the fields the user has already filled in.
  // `canGenerate` is true once there is a title and a start time — the same
  // two fields publishing requires — and it also gates the offscreen canvas,
  // so the canvas is always mounted whenever the button is tappable.
  ...
  async function pickPoster() {
    ...
    if (!result.canceled) {
      ...
      // A real photo beats a generated poster — drop the generated bytes so
      // publish uploads the picked file and not a stale render.
      setGeneratedPoster(null);
    }
  }

  /**
   * Make a poster out of what the form already knows. Not a separate screen
   * and not a choice of templates: one tap fills the same cover-image tile the
   * photo picker fills, and the result can be replaced by tapping the tile.
   */
  async function handleGeneratePoster() {
    try {
      const generated = await poster.generate();
      setGeneratedPoster(generated);
      setPosterUri(generated.dataUri);
  ...
  async function doPublish() {
    ...
    try {
      ...
      if (generatedPoster) {
        // Generated posters upload their PNG bytes directly — a data: URI
        // cannot be round-tripped through fetch() on React Native, and this
        // path is where the blank-output guard runs on the exact bytes.
        posterUrl = await uploadGeneratedEventPoster(
          user.id,
          eventId,
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* The poster canvas, mounted offscreen. `Svg.toDataURL()` snapshots a
