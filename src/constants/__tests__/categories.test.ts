...
describe('legacy category data keeps working', () => {
  ...
  // What "recognised" means, and why this is not `toHaveLength(2)` any more.
  //
  // It was, and that was a PROXY: while every shortlist held exactly two
  // families, "length 2" and "not the fallback" were the same statement. The
  // 2026-08-19 merge added `axial` and `technical` and some shortlists became
  // three long, so the proxy started failing on categories that are perfectly
  // well recognised — and, worse, it would have gone on passing if a shortlist
  // had silently shrunk to two of the SIX families by accident.
  //
  // The real property is stated directly below: a recognised category must not
  // fall back to the full family set, which is what "the poster engine stopped
  // understanding this event" actually looks like.
  const posterFallback = familyShortlist([]);
  const coverFallback = coverFamilyShortlist([]);

  it.each(OLD_FOURTEEN)('%s still resolves to a real poster shortlist', (old) => {
    const list = familyShortlist([old]);
    expect(list).not.toEqual(posterFallback);
    expect(list.length).toBeGreaterThanOrEqual(2);
    expect(list.length).toBeLessThan(posterFallback.length);
  });

  it.each(OLD_FOURTEEN)('%s still resolves to a real cover shortlist', (old) => {
    const list = coverFamilyShortlist([old]);
    expect(list).not.toEqual(coverFallback);
    expect(list.length).toBeGreaterThanOrEqual(2);
  });

  it('every current category resolves to a real poster shortlist too', () => {
    for (const cat of EVENT_CATEGORIES) {
      expect(familyShortlist([cat])).not.toEqual(posterFallback);
      expect(coverFamilyShortlist([cat])).not.toEqual(coverFallback);
    }
  });
